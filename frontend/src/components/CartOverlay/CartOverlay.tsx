...
import { AttributeSet } from '../../types/index';
...
const CartOverlay: React.FC<CartOverlayProps> = ({ onClose, contentRef }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderItems = cartState.items.map((item) => ({
      ...
      chosenAttributes: Object.entries(item.chosenAttributes).map(([name, value]) => ({
        name,
        value,
      })),
    }));
  ...
  return (
    <div className="cart-overlay" data-testid="cart-overlay" onClick={onClose}>
      <div className="cart-overlay__content" ref={contentRef} onClick={(e) => e.stopPropagation()}>
        <h3>
          My Bag,
          ...
          {cartState.items.map((item, idx) => (
            <div key={idx} className="cart-item">
              <div className={'cart-item__options'}>
                ...
                {item.attributes &&
                  item.attributes.map((attrSet: AttributeSet) => {
